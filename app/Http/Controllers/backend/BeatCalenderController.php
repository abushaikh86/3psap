...
class BeatCalenderController extends Controller
{
    ...
    {
        $sales_execu_data = AdminUsers::get();
        $beats = Beat::pluck('beat_name', 'beat_id');

        $all_data = getCommonArrays();

        if ($request->ajax()) {
            $data = BeatCalender::with('get_beat')->orderByRaw("FIELD(beat_day, '" . implode("', '", $all_data['days']) . "')")
                ->orderByRaw("FIELD(beat_week, '" . implode("', '", $all_data['week']) . "')")
                ->orderByRaw("FIELD(beat_month, '" . implode("', '", $all_data['month']) . "')")
                ->orderByRaw("FIELD(beat_year, '" . implode("', '", $all_data['year']) . "')")
                ->get();
            ...
            return DataTables::of($data)
                ...
                ->addColumn('action', function ($data) {
                    ...
                    // if (request()->user()->can('Update Beat Calender')) {
                    // dd("yes");
                    ...
                    // }
                    // if (request()->user()->can('Delete Beat Calender')) {
        ...
        return view('backend.beatcalender.index', compact('all_data', 'beats'));
    }
    ...
    public function store(Request $request)
    {
        ...
        // $outlet_data = implode(',', $request->outlet);
        // $days_data = implode(',', $request->beat_day);
        // dd($outlet_data);
        $validate_beat = BeatCalender::where([
            'beat_id' => $request->beat_id,
            'beat_year' => $request->beat_year, 'beat_month' => $request->beat_month,
            'beat_week' => $request->beat_week
        ])->first();

        if (!empty($validate_beat)) {
            return redirect()->back()->with('error', 'This beat is already given in this week');
        }
