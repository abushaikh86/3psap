...
class PurchaseorderController extends Controller
{
        ...
        if ($request->ajax()) {
            ...
            return DataTables::of($purchaseorder)
                ...
                ->addColumn('action', function ($purchaseorder) {
                    ...
                    if (request()->user()->can('Clone Purchase Order') && $purchaseorder->status != 'close') {
                ...
                ->addColumn('new_action', function ($purchaseorder) {
                    $actionBtn = ' <div class="dropdown">
                    <button class="btn btn-sm btn-secondary dropdown-toggle" type="button" data-bs-toggle="dropdown"
                    aria-expanded="false">Action</button>
                    <ul class="dropdown-menu" id="action_dropdown_menu">';
                    if (request()->user()->can('View Purchase Order')) {
                        $actionBtn .= ' <li> <a href="' . route('admin.purchaseorder.view', ['id' => $purchaseorder->purchase_order_id]) . '"
                    class="dropdown-item" id="action_dropdown_item">
                    <span class="badge rounded-pill bg-primary">View</span></a></li> ';
                    }
                    if (request()->user()->can('Clone Purchase Order') && $purchaseorder->status != 'close') {
                        $actionBtn .= '<li><a href="' . route('admin.purchaseorder.creategr', ['id' => $purchaseorder->purchase_order_id]) . '
                     " class="dropdown-item" id="action_dropdown_item" >
                     <span class="badge rounded-pill bg-info">Create GR</span></a></li>';
                    }
                    if (request()->user()->can('Update Purchase Order') && $purchaseorder->status == 'open') {
                        $actionBtn .= ' <li> <a href="' . route('admin.purchaseorder.edit', ['id' => $purchaseorder->purchase_order_id]) . '
                     " class="dropdown-item" id="action_dropdown_item">
                     <span class="badge rounded-pill bg-success">Edit</span></a></li> ';
                    }
                    if (request()->user()->can('Delete Purchase Order')) {
                        $actionBtn .= '<li><a href="' . route('admin.purchaseorder.delete', ['id' => $purchaseorder->purchase_order_id]) . '"
    ...
    public function creategr($id)
    {
        ...
        if (!isset($goods_receipt_exist)) {
            ...
            $log = ['module' => 'GR Receipt', 'action' => 'GR Receipt Created', 'description' => 'GR Receipt Created: ' . $goods_receipt->bill_no];
            captureActivity($log);
