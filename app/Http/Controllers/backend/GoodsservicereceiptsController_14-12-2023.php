...
class GoodsservicereceiptsController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        if ($request->ajax()) {
            $goodsservicereceipts = GoodsServiceReceipts::with('get_partyname')->get();
            ...
            return DataTables::of($goodsservicereceipts)
                ...
                ->addColumn('action', function ($goodsservicereceipts) {
                    ...
                    if (request()->user()->can('Clone Goods Receipt PO')) {
                     ...
                     " class="btn btn-sm btn-info" title="Create A/P Invoice" onclick="return confirm(`Are you sure you want to Create A/P Invoice from this GR ?`)">
                ...
                ->addColumn('new_action', function ($goodsservicereceipts) {
                    ...
                    if (request()->user()->can('View Goods Receipt PO')) {
                        $actionBtn .= ' <li> <a href="' . route('admin.goodsservicereceipts.view', ['id' => $goodsservicereceipts->goods_service_receipt_id]) . '"
                    class="dropdown-item" id="action_dropdown_item">
                    <span class="badge rounded-pill bg-primary">View</span></a></li> ';
                    }
                    if (request()->user()->can('Clone Goods Receipt PO')) {
                        $actionBtn .= '<li><a href="' . route('admin.goodsservicereceipts.createapinvoice', ['id' => $goodsservicereceipts->goods_service_receipt_id]) . '
                     " class="dropdown-item" id="action_dropdown_item" onclick="return confirm(`Are you sure you want to Create A/P Invoice from this GR ?`)">
                     <span class="badge rounded-pill bg-info">Create A/P Invoice</span></a></li>';
                    }
                    if (request()->user()->can('Update Goods Receipt PO')) {
                        if (!$goodsservicereceipts->is_inventory_updated) {
                            $actionBtn .= ' <li> <a href="' . route('admin.goodsservicereceipts.edit', ['id' => $goodsservicereceipts->goods_service_receipt_id]) . '
                     " class="dropdown-item" id="action_dropdown_item">
                     <span class="badge rounded-pill bg-success">Edit</span></a></li>';
                        }
                    }
                    if (request()->user()->can('Delete Goods Receipt PO')) {
                        $actionBtn .= '<li><a href="' . route('admin.goodsservicereceipts.delete', ['id' => $goodsservicereceipts->goods_service_receipt_id]) . '"
            ...
            if ($goodsservicereceipt->update()) {
                ...
                if (isset($request->old_invoice_items)) {
                    ...
                    foreach ($filteredItems as $row) {
                        ...
                        // $good_bin = BinManagement::whereHas('get_bin', function ($query) use ($row) {
                        //     $query->where('name', 'Goods')->whereIn('bin_type_id', function ($subQuery) use ($row) {
                        //         $subQuery->select('bin_id')->from('bin_mangement')->where('warehouse_id', $row['storage_location_id']);
                        //     });
                        // })->first();
    ...
    public function create_remaining_gr($id, $existing_data, $existing_items, $new_quantity)
    {

        // dd($existing_items->toArray(),$new_quantity);
        // dd($new_quantity);
        // set job counter
        ...
        // //update status
        // $old_goodsservicereceipt = GoodsServiceReceipts::where('goods_service_receipt_id', $existing_data['goods_service_receipt_id'])->first();
        // $old_goodsservicereceipt->is_gr_done = 1;
        // $old_goodsservicereceipt->is_inventory_updated = 1;
        // $old_goodsservicereceipt->save();
