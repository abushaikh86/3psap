...
class ApiController extends Controller
{
  ...
  public function get_daily_beats()
  {
    ...
    $beats = BussinessPartnerMaster::with('get_beat')
      ...
      ->where('salesman', $_REQUEST['user_id'])
      ->get()
  ...
  public function get_outlets()
  {
    ...
    $outlets = BussinessPartnerMaster::where(['business_partner_type' => '1', 'beat_id' => $_REQUEST['beat_id'], 'salesman' => $_REQUEST['user_id'], 'is_converted' => 1])->get()->toArray();
    // dd($outlets);
  ...
  {
    $orders = OrderBooking::where(['party_id' => $_REQUEST['outlet_id']])->orderby('created_at', 'desc')->get()->toArray();
  ...
  public function get_item_auto()
  {
    ...
    $item_code = $data[0]->item_code ?? $data[0]->name;
    $pricing_price = 0;
    if (!empty($item_code)) {
