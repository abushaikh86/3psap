<script type="text/javascript">
    ...
    function updateTotals() {
        ...
        $('input.select-row:checked').each(function() {
            ...
            //     var cgstAmount = parseFloat($row.find('td:eq(9)')
            // .text()); // Assuming column 8 contains the CGST rate
            //     var sgstAmount = parseFloat($row.find('td:eq(11)')
            // .text()); // Assuming column 9 contains the SGST/UTGST Amount
            //     var igstAmount = parseFloat($row.find('td:eq(13)')
            // .text()); // Assuming column 10 contains the IGST rate
            //     var totalValue = parseFloat($row.find('td:eq(14)')
            // .text()); // Assuming column 11 contains the total value
            // console.log(parseFloat($row.find('td:eq(5)').text()));
            netTotal += amount;
    ...
    function get_series(party_id, edit_series_id = null) {
        ...
        if (party_id) {
            $.ajax({
                ...
                url: '<?php echo e(route('admin.get_series')); ?>', // Replace with your server endpoint
                data: {
                    party_id: party_id,
                },
                dataType: 'json',
    ...
    function get_doc_numbers(party_id, selectedTransactionType, edit_doc_no = null) {
        if (party_id && selectedTransactionType) {
            $.ajax({
                ...
                url: '<?php echo e(route('admin.get_doc_numbers')); ?>', // Replace with your server endpoint
                data: {
                    party_id: party_id,
    ...
    function get_bill_bookings(selectedTransactionType, bill_booking_item_ids = null, netTotal = null, taxTotal = null,
        ...
        if (selectedTransactionType) {
            ...
            $.ajax({
                ...
                url: '<?php echo e(route('admin.get_bill_bookings')); ?>', // Replace with your server endpoint
                data: {
                ...
                success: function(data) {
                    ...
                    // console.log(data);
                    // console.log(data[0].billbooking_items);
                    ...
                    data.forEach(function(item, index) {
                        ...
                        // newRow += '<td>' + (item.billbooking_items[0].cgst_rate || ' ') + '</td>';
                        // newRow += '<td style="display:none">' + (item.billbooking_items[0]
                        //     .cgst_amount || ' ') + '</td>';
                        // newRow += '<td>' + (item.billbooking_items[0].sgst_utgst_rate || ' ') +
                        //     '</td>';
                        // newRow += '<td style="display:none">' + (item.billbooking_items[0]
                        //     .sgst_utgst_amount || ' ') + '</td>';
                        // newRow += '<td>' + (item.billbooking_items[0].igst_rate || ' ') + '</td>';
                        // newRow += '<td style="display:none">' + (item.billbooking_items[0]
                        //     .igst_amount || ' ') + '</td>';
                        // newRow += '<td>' + item.billbooking_items[0].total_value + '</td>';
                        newRow += '</tr>';
    ...
    //     get_doc_numbers(party_id,selectedTransactionType);
    // });


    $(document).ready(function() {
        var party_id = $('#party_id').val();
        var edit_series_id = '<?php echo e($model->series_no ?? ''); ?>';
        var edit_doc_no = '<?php echo e($model->doc_no ?? ''); ?>';
        var bill_booking_item_ids = '<?php echo e($model->bill_booking_item_ids ?? ''); ?>';

        // alert(bill_booking_item_ids);

        var netTotal = $('input.netTotal').val() == 0 ? '<?php echo e($model->net_total ?? ''); ?>' : $('input.netTotal')
            .val();
        var taxTotal = $('input.taxTotal').val() == 0 ? '<?php echo e($model->tax_total ?? ''); ?>' : $('input.taxTotal')
            .val();
        var grossTotal = $('input.grossTotal').val() == 0 ? '<?php echo e($model->gorss_total ?? ''); ?>' : $(
        ...
        // get_series(party_id, edit_series_id);
        // get_doc_numbers(party_id, edit_series_id, edit_doc_no);
...
<?php /**PATH C:\wamp64\www\3psap\resources\views/backend/dynamic_list_banking_payment_script.blade.php ENDPATH**/ ?>