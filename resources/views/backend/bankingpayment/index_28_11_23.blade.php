                                    ...
                                    <tbody>
                                        @if (isset($details) && count($details) > 0)
                                            ...
                                            @foreach ($details as $data)
                                                <tr>
                                                    <td>{{ $srno }}</td>
                                                    <td>{{ $data->get_partyname->bp_name ?? '' }}</td>
                                                    <td>{{ $data->doc_no }}</td>
                                                    {{-- <td>{{ $data->get_series_data->series_number }}</td> --}}
                                                    <td>{{ $data->gorss_total }}</td>
                                                    <td>{{ $data->posting_date }}</td>

                                                    <td>

                                                        @can('Update Banking Payments')
                                                            <a href="{{ url('admin/bankingpayment/edit/' . $data->banking_payment_id) }}"
                                                                class="btn btn-primary" title="Edit"><i
                                                                    class="feather icon-edit"></i></a>
                                                        @endcan

                                                        @can('Delete Banking Payments')
                                                            {!! Form::open([
                                                                'method' => 'GET',
                                                                'url' => ['admin/bankingpayment/delete', $data->banking_payment_id],
