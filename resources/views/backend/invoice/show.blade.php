...
<div class="col-sm-12 text-sm-end mb-1">
  ...
  @include('backend.invoice.invoice_format_split', ['roles'=>$roles,'invoice'=>$invoice,'bill_address'=>$bill_address,'ship_address'=>$ship_address,'contact'=>$contact,'party'=>$party,'download'=>false])
  @else
  @include('backend.invoice.invoice_format', ['roles'=>$roles,'invoice'=>$invoice,'bill_address'=>$bill_address,'ship_address'=>$ship_address,'contact'=>$contact,'party'=>$party,'download'=>false])
  @endif
      <div class="row">
...
<script>
    ...
    <script type="text/javascript">
        ...
        {
          //alert(data);
            var mywindow = window.open('', '{{ $invoice->bill_no }}', 'height=600,width=800');
            mywindow.document.write('<html><head><title>{{ $invoice->bill_no }}</title>');
