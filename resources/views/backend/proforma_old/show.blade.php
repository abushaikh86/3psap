...
<div class="content-header row">
  <div class="content-header-left col-md-6 col-12 mb-2">
    <h3 class="content-header-title">Sale Order</h3>
...
<div class="col-sm-12 text-sm-end mb-1">
  ...
  @include('backend.proforma.invoice_format_split', ['roles'=>$roles,'invoice'=>$proforma,'party'=>$party,'download'=>false])
  @else
  @include('backend.proforma.invoice_format', ['roles'=>$roles,'invoice'=>$proforma,'party'=>$party,'download'=>false])
  @endif
      <div class="row">
        <div class="col-lg-12 text-center">
  <a href="javascript:void(0);" class="btn btn-primary" onclick="PrintElem('.printable');">Print</a>
          <a href="{{ route('admin.proforma') }}" class="btn btn-secondary">Back</a>
        </div>
      </div>
...
<script>
    ...
    <script type="text/javascript">
        ...
        {
          //alert(data);
            var mywindow = window.open('', '{{ $proforma->bill_no }}', 'height=600,width=800');
            mywindow.document.write('<html><head><title>{{ $proforma->bill_no }}</title>');
