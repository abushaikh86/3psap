  ...
  <div class="row">
	...
	<div class="row">
        ...
        <div class="order-address pl-4 pt-3">
          ...
          <h2>My Shopping Cart ({{ count(session('cart')) }} Item{{ (count(session('cart'))>1)?'s':'' }})</h2>

        </div>

        <hr class="w-100">
        <form method="POST" action="{{url('cart/updatecart')}}">
        {{ csrf_field() }}
        @php
        ...
        @endphp
				<div class="delivery-details-box cart-box mb-2">
                  ...
                  @php
                     // die();
                  @endphp
									<p><span class="delivery-pro-type">{{ $item->product_title }}</span></p>
                  <p>
                    @if($item->product_discounted_price!=null && $item->product_discount!=0)
                      <span class="delivery-pro-price">₹ {{ isset($item->product_discount)?$item->product_discounted_price:$item->{$join_table}->product_price }}</span>
                      <span class="delivery-pro-original">₹ {{ $item->product_price }}</span>
                      @php
                        $item_price = $item->product_discounted_price*$item->qty;
                        $item_mrp = $item->product_price*$item->qty;

                        $item_discount = $item_mrp-$item_price;
                        @endphp

                    @elseif($item->product_discounted_price!=null && $item->product_discount==0)
                      <span class="delivery-pro-price">₹ {{$item->product_discounted_price}}</span>
                      @php
                        $item_price = $item->product_discounted_price*$item->qty;
                        $item_mrp = $item->product_discounted_price*$item->qty;
                        $item_discount = $item_mrp-$item_price;
                      @endphp
                    @else
                      <span class="delivery-pro-original">₹ {{$item->product_price}}</span>
                      ...
                      @endphp
                    @endif
                    @if($item->product_discount_type!=null && $item->product_discount_type=='percent' && $item->product_discount!=0)
                      <span class="delivery-pro-discount">({{ $item->product_discount }}% OFF)</span>
                    @elseif($item->product_discount_type!=null && $item->product_discount_type=='flat' && $item->product_discount!=0)
                      <span class="delivery-pro-discount">(₹ {{ $item->product_discount }} OFF)</span>
                    @endif
                  </p>
									@if(isset($item->product_discount) && $item->product_discount!=0)
                    <p><span class="delivery-pro-description">You are saving  ₹ {{ $item->product_price-$item->product_discounted_price }} on this Order</span></p>
                  @endif
								</div>
							</div>
						</div>
							...
							<div class="col-md-5 col-sm-5 col-12 ">
								<div class="delivery-period">
									...
									<div class="delivery-product-size">
										<p>{{ isset($item->size)?$item->size->size_name:'FREE' }}</p>
									</div>
								</div>
							</div>
							<div class="col-md-7 col-sm-7 col-12 ">
								<div class="delivery-period">
									<p>Quantity: </p>
										...
										<input type="text" class="qtyValue" name="cart_data[{{ $item->id }}][qty]" value="{{ $item->qty }}">
										<i class="fa fa-plus increaseQty"></i>
									</div>
								</div>
							</div>
              @if($item->product_type == 'configurable')
                <input type="hidden" name="cart_data[{{ $item->id }}][product_variant_id]" id="product_variant_id" value="{{$item->product_variant_id}}">
              @endif
              <input type="hidden" name="cart_data[{{ $item->id }}][product_type]" id="product_type" value="{{$item->product_type}}">
              @if(isset($item->{$join_table}) && $item->product_qty < $item->qty)
              @php
              ...
              <div class="col-md-7 col-sm-7 col-12">
                Please Change Quantity Only {{$item->product_qty}} left!
              </div>
              ...
              @endif
						...
						<div class="row delivery-details-row shopping-btns ">
								<div class="col-md-6 col-sm-6 col-6 text-center">
									<p> <a href="{{ url('/cart/deleteitem') }}/{{ $item->id }}">REMOVE SHOPPING</a></p>
								</div>
								<div class="col-md-6 col-sm-6 col-6 text-center">
